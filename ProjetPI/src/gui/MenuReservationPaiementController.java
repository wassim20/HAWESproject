...
public class MenuReservationPaiementController implements Initializable {
    ...
    private void modifierReservation(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("AfficherReservationsNTFXML.fxml"));
            Parent root =loader.load();
            AfficherReservationsNTFXMLController arnt = loader.getController();
            //ap.setReservation(r);
            btnaffr.getScene().setRoot(root);
        } catch (IOException ex) {
            System.out.println(ex.getMessage());
    ...
    private void supprimerReservation(ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("SuppressionReservationFXML.fxml"));
            Parent root =loader.load();
            SuppressionReservationFXMLController spr = loader.getController();
            btnsuppr.getScene().setRoot(root);
        } catch (IOException ex) {
            System.out.println(ex.getMessage());
        }   
    }
